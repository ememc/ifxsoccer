...
const normalizeProgramItem = (item: ProgramRecord, index: number): ProgramItem | null => {
    ...
    const description = getStringValue(item, ["program_description", "description"]);
    const applyUrl = getStringValue(item, ["program_apply", "apply_url", "apply", "link"]) || "#";
    const id = getStringValue(item, ["program_id", "id"]) || `${slugify(title)}-${index}`;
    const slug = slugify(title) || id.toLowerCase();
    const imageUrl = getProgramHeroImage(item) || FALLBACK_PROGRAM_IMAGE;
